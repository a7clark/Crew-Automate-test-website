// usage: node shot-section.mjs <url> <css-selector> <label>
...
const [url, selector, label] = process.argv.slice(2);
const existing = readdirSync(outDir).map(f => /^screenshot-(\d+)/.exec(f)).filter(Boolean).map(m => parseInt(m[1], 10));
